<template>
  <div class="starting-page">
    ...
    <div class="address-list-wrapper"
         v-if="addresses.length && isAddresses">
      <address-list :data="addresses"
                    @choose="chooseItem"></address-list>
    </div>
    <map id="map-wsjc"
         class="map-wsjc"
         :latitude="latitude"
      ...
      <cover-view class="center-marker">
        ...
        <cover-image class="inverted-triangle"
                     src="/static/img/triangle-down.png"></cover-image>
        <cover-image class="img-center"
                     src="/static/img/marker2.png"></cover-image>
      </cover-view>

      <cover-view class="address">
        <cover-image class="img-address"
        ...
        <cover-view class="btn-address"
                    @click.stop="setStartPlace">确定</cover-view>
      </cover-view>
    </map>
  </div>
</template>
...
<script>
import { QQ_MAP_key as qqMapKey } from '../../common/constant/constant'
...
export default {
  ...
  computed: {
    ...mapState(['curNavIndex', 'curCity', 'startPlace', 'startFormattedPlace', 'startPosition'])
  },
  components: {
    SearchBar,
    AddressList
  ...
  methods: {
    ...
    chooseCity() {
      wx.navigateTo({ url: '/pages/city-choose/city-choose' })
    },
    search(value) {
    ...
    updateCars() {
      ...
      for (let i = 1; i <= carNum; i++) {
        ...
        let car = {
          id: 0,
          iconPath: '/static/img/car/cart.png',
          latitude: 0,
          longitude: 0,
          width: 35,
          height: 15
  ...
  onShow() {
    this.initLocation()
    this.mapCtx = wx.createMapContext('map-wsjc') // 地图组件的id
    this.updateCars()
  }
}
</script>
